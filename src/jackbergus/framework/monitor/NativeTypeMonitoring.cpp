...
// #endif
            uint64_t record_offset, uint64_t sizeof_, const std::unordered_map<uint64_t, std::string>& map): name(name), casusu(casusu),
// #if __cplusplus >= 202302L
        type_i(idx),
// #endif
        idx{record_offset}, sizeof_(sizeof_), forEnumValueToNameMapping(map) {}
    } // framework
} // jackbergus